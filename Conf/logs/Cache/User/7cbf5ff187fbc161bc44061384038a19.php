...
<div class="cLineB">
 ...
 <a href="<?php if($fid != 0): echo U('Classify/add',array('fid'=>$fid,'grouptype'=>$_GET['grouptype'])); else: echo U('Classify/add',array('grouptype'=>$_GET['grouptype'])); endif; ?>" class="btnGrayS vm bigbtn">添加<?php if($fid != 0): ?>【<?php echo ($thisClassify["name"]); ?>】子<?php endif; ?>分类</a>　
</div>
...
<td class="norightborder" width="120">
	...
	<a href="<?php echo U('Classify/index',array('fid'=>$vo['id'],'token'=>$_SESSION['token'],'grouptype'=>$_SESSION['grouptype']));?>">子分类</a>
  <a href="<?php if ($vo['fid']==0){ echo U('Classify/flash',array('tip'=>3,'id'=>$vo['id'],'token'=>$_SESSION['token'],'grouptype'=>$_SESSION['grouptype'])); }else{ echo U('Classify/flash',array('tip'=>3,'id'=>$vo['id'],'fid'=>$vo['fid'],'token'=>$_SESSION['token'],'grouptype'=>$_SESSION['grouptype'])); }?>">幻灯片</a>
  <a href="<?php if ($vo['fid']==0){ echo U('Classify/essay',array('id'=>$vo['id'],'token'=>$_SESSION['token'],'grouptype'=>$_SESSION['grouptype'])); }else{ echo U('Classify/essay',array('id'=>$vo['id'],'fid'=>$vo['fid'],'token'=>$_SESSION['token'],'grouptype'=>$_SESSION['grouptype'])); }?>">文章</a>
	<a href="<?php if ($vo['fid']==0){ echo U('Classify/edit',array('id'=>$vo['id'],'token'=>$_SESSION['token'],'grouptype'=>$_SESSION['grouptype'])); }else{ echo U('Classify/edit',array('id'=>$vo['id'],'fid'=>$vo['fid'],'token'=>$_SESSION['token'],'grouptype'=>$_SESSION['grouptype'])); }?>">编辑</a>
	<a href="<?php if ($vo['fid']==0){ echo U('Classify/del',array('id'=>$vo['id'])); }else{ echo U('Classify/del',array('id'=>$vo['id'],'fid'=>$vo['fid'],'fid'=>$vo['fid'])); }?>">删除</a>
	</nobr>
</td>
