...
class sitePage extends front{
	...
	function __construct(){
		...
		if (!$this->site){
			$this->site=$this->site_db->get_one(array('url'=>$_SERVER['HTTP_HOST']));
			$this->token=$this->site['token'];
		}
		//if (!$this->site) exit('no site');
	...
	function index(){
		...
		*/
		...
		$shareStr=$this->shareScript($this->site['name'],$this->site['picurl'],'','http://'.$_SERVER['HTTP_HOST'].'/cms/index.php?token='.$this->token);
		$this->assign('shareScript',$shareStr);
		//display
	...
	function content(){
		...
		$shareStr=$this->shareScript($this->article->title,$this->article->thumb,'','http://'.$_SERVER['HTTP_HOST'].'/cms/index.php?m=site&c=home&a=content&contentid='.$this->article->id.'&token='.$this->token);
		$this->assign('shareScript',$shareStr);
		//
			...
			foreach ($plugmenus as $pm){
				switch ($pm['name']){
					...
					case 'membercard':
						...
						if (!$pm['url']){
							if($card==false){
								$pm['url']='/index.php?g=Wap&m=Card&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
...
marker1.addEventListener("click", function(){this.openInfoWindow(infoWindow1);});
	...
	public function getLink($url){
		...
		if (strExists($url,'刮刮卡')){
			...
			$link=PIGCMS_URL.'/index.php?g=Wap&m=Card&a=index&token='.$this->token.'&wecha_id='.$this->wechat_id;
	...
	public function shareScript($title,$thumb,$intro,$link){
		$scriptData='<script type="text/javascript">			window.shareData = {  
					"moduleName":"Index",
					"moduleID": 0,
					"imgUrl": "'.$thumb.'", 
					"timeLineLink": "'.$link.'",
	...
	$.post('/index.php?g=Wap&m=Share&a=shareData&token=".$this->token."&wecha_id=".$this->wechat_id."',submitData,function (data) {},'json')
}
        </script>";
		return $scriptData.$this->shareScript;
	}
}