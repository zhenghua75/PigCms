	...
	public function index($orderid, $paytype = '', $third_id = ''){
		...
		if ($order = M('Hotels_order')->where(array('orderid' => $orderid))->find()) {
			...
			if ($order['paid']) {
				...
				$model->sendTempMsg('OPENTM202521011', array('href' => U('Hotels/my',array('token' => $order['token'], 'wecha_id' => $order['wecha_id'])), 'wecha_id' => $order['wecha_id'], 'first' => '预订房间提醒', 'keyword1' => $orderid, 'keyword2' => date("Y年m月d日H时i分s秒"), 'remark' => '预订房间成功，感谢您的光临，欢迎下次再次光临！'));
		
			}
			header('Location:/index.php?g=Wap&m=Hotels&a=my&token='.$order['token'].'&wecha_id='.$order['wecha_id']);
