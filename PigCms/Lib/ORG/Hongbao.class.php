...
class Hongbao extends Hongbao_common{
	...
	function __construct($config) {
		if(empty($config['token']) || empty($config['openid']) || empty($config['money'])){
			return json_encode(array('status'=>'FAIL','msg'=>'Token,用户openid,金额不能为空'));
		}
		//获取微信配置信息
		$info = M('alipay_config')->where(array('token'=>$config['token'],'open'=>1))->find();
		if(empty($info)){
			return json_encode(array('status'=>'FAIL','msg'=>'Token错误'));
		}
		$weixin = unserialize($info['info']);
		if(empty($weixin['weixin']['key']) || empty($weixin['weixin']['mchid']) || empty($weixin['weixin']['new_appid'])){
			return json_encode(array('status'=>'FAIL','msg'=>'密钥,商户号或公众号未获取到'));
		}
		$this->token     = $config['token'];
		$this->key 		 = trim($weixin['weixin']['key']);
		$this->mchid 	 = trim($weixin['weixin']['mchid']);
		$this->wxappid 	 = trim($weixin['weixin']['new_appid']);
