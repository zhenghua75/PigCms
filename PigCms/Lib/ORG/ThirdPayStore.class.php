	...
	public function index($orderid, $paytype = '', $third_id = ''){
		if ($order = M('Product_cart')->where(array('orderid' => $orderid))->find()) {
			...
			if ($order['paid']) {
				...
				$model->sendTempMsg('OPENTM202521011', array('href' => U('Store/my',array('token' => $order['token'], 'wecha_id' => $order['wecha_id'], 'twid' => $order['twid'])), 'wecha_id' => $order['wecha_id'], 'first' => '购买商品提醒', 'keyword1' => $orderid, 'keyword2' => date("Y年m月d日H时i分s秒"), 'remark' => '购买成功，感谢您的光临，欢迎下次再次光临！'));
			}
