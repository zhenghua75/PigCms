...
class UserAction extends BaseAction
{
	...
	protected function _initialize()
	{
		...
		foreach ($Afunc as $tk => $tv ) {
			$vvv = ($trans[$tv["funname"]] ? ucfirst($trans[$tv["funname"]]) : ucfirst($tv["funname"]));
		...
		if (MODULE_NAME != "Upyun") {
			...
			if ($_SESSION["administrator"] || $_SESSION["agentid"] || $_SESSION["uid"] || $_SESSION["wapupload"]) {
			}
			else if ($_POST["PHPSESSID"]) {
		...
		if ((session("companyLogin") == 1) && !in_array(MODULE_NAME, array("Attachment", "Repast", "Upyun", "Hotels", "Store", "Classify", "Catemenu", "DishOut", "LivingCircle"))) {
			if ((MODULE_NAME == "Recognition") && (ACTION_NAME == "get_Wxticket")) {
			}
			else {
		...
		$this->assign("__access_remind__", "<div style=\"color:red\"> &nbsp;（该选项仅适用于认证服务号和认证订阅号）</div>");
	}
