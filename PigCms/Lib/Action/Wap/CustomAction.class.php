...
class CustomAction extends WapAction{
	...
	public function index(){
		...
		if(IS_POST){
			...
			if($this->thisForm['theme']=='xbh'){
				if($spoor<>0 && $this->wecha_id<>'null'){
					$this->redirect(U('Custom/spoor', array('token' => $this->token,'id'=>$this->thisForm['set_id'],'wecha_id'=>$this->wecha_id,'id'=>$this->thisForm['set_id'])));
				}else{
	                $group = defined('GROUP_NAME')?GROUP_NAME.'/':'';
	                $this->display(TMPL_PATH.$group.$this->thisForm['theme'].'/'.MODULE_NAME.(defined('GROUP_NAME')?C('TMPL_FILE_DEPR'):'/').ACTION_NAME.C('TMPL_TEMPLATE_SUFFIX'));
				}
			}elseif($this->thisForm['theme']){
                $group   =  defined('GROUP_NAME')?GROUP_NAME.'/':'';
                $this->display(TMPL_PATH.$group.$this->thisForm['theme'].'/'.MODULE_NAME.(defined('GROUP_NAME')?C('TMPL_FILE_DEPR'):'/').ACTION_NAME.C('TMPL_TEMPLATE_SUFFIX'));
            }else{
                $this->display();
