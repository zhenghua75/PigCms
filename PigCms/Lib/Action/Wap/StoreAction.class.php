...
// 			}
			...
			$model->sendTempMsg('TM00184', array('href' => U('Store/my',array('token' => $this->token, 'wecha_id' => $wecha_id)), 'wecha_id' => $wecha_id, 'first' => '购买商品提醒', 'ordertape' => date("Y年m月d日H时i分s秒"), 'ordeID' => $orderid, 'remark' => '购买成功，感谢您的光临，欢迎下次再次光临！'));
			if ($totalprice) {
...
// 	    }
	...
	public function register()
	{
		if (IS_POST) {
			...
			if ($uid) {
				...
				if ($_GET['retm']=="card") {
					$this->success('注册成功', U('Card/index', array('token' => $this->token, 'wecha_id' => $wecha_id, 'twid' => $twid)));
				} else {
					$this->success('注册成功', U('Store/cats', array('token' => $this->token, 'wecha_id' => $wecha_id, 'twid' => $twid, 'cid' => $this->_cid)));
				}
			} else {
	...
	{
        if (session('twid')) {
        	$this->assign('twid', session('twid'));
        	$this->mytwid=session('twid');	
        	$this->_twid=session('twid');
		...
		if (IS_POST) {
			...
			if (empty($userInfo)) {
				...
				$this->_twid=$userInfo['twid'];
				$this->mytwid=$userInfo['twid'];
				$this->assign('twid', $userInfo['twid']);
				$callbackurl = session('callbackurl');
				if ($_GET['retm']=="card") {
					$this->success('登录成功', U('Card/index', array('token' => $this->token, 'wecha_id' => $this->_twid, 'twid' => $this->_twid)));
				} elseif ($callbackurl) {
...
// 		echo $this->mytwid;die;
	...
	public function logout()
	{
		...
		$this->redirect(U('Store/cats',array('token' => $this->token, 'wecha_id' => '', 'twid' => '')));
	}
