...
class ReplyAction extends WapAction{
    ...
    public function __construct(){
     ...
     public function index(){ //显示数据
         ...
         $leave_model =M("leave");
            ...
            $where = array("token"=>$this->token,'checked'=>1,'type'=>'m_leave');
    ...
    public function leave(){//留言信息插入处理
        ...
        $lasttime = $leave_model->where(array("token"=>$this->token,'type'=>'m_leave'))->getField("max(time)");//获得准备数据 是否与数据库中数据留言是同一人
     ...
     public function reply(){//回复信息处理
         ...
         //
          $leave_model =M("leave");
          $thisMessage=$leave_model->where(array('id'=>intval($message_id),'type'=>'m_leave'))->find();
          if (!$thisMessage){
