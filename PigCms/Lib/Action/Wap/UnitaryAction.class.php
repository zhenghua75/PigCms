...
class UnitaryAction extends WapAction{
	...
	public function wxregister(){
		...
		if($this->isSubscribe() == false){
			$this->assign("gz","yes");
		}/*elseif(($register == 1 && empty($this->fans)) || ($wxregister == 0 && empty($this->wecha_id))){
			if($this->wecha_id) {
				$href = U('Index/memberReg',array('token'=>$this->token));
			}else{
				$href = U('Index/memberLogin',array('token'=>$this->token));
			}
			$this->success("您是游客身份，请先登录/注册",$href);
			exit;
		}*/
	}
	...
	public function dobuy(){
		...
		if($id_order > 0){
			...
			if($update_cart > 0){
				...
				//$this->redirect("Unitary/payReturn",array("token"=>$this->token,"price"=>$total,"wecha_id"=>$this->wecha_id,"from"=>"Unitary","orderid"=>$save_order['orderid'],'single_orderid'=>$save_order['orderid'],'notOffline'=>1));
			}
		}
	}
