...
class UserinfoAction extends WapAction{
	...
	public function index(){
		...
		$userinfoexp=D('Userinfoexp')->where(array('uid'=>$userinfo['id']))->find();
		$area['province']=D('China_address')->where(array('region_type'=>1))->select();
		if($userinfoexp){
			$area['city']=D('China_address')->where(array('region_type'=>2,'parent_id'=>$userinfoexp['province']))->select();
			$area['country']=D('China_address')->where(array('region_type'=>3,'parent_id'=>$userinfoexp['city']))->select();
		}
		...
		$this->assign('userinfoexp',$userinfoexp);
		$this->assign('indusid','userinfo');
		//redirect url
		...
		if(IS_POST){
			...
			$province=$this->_post('province')? $this->_post('province') : '';
			$city=$this->_post('city')? $this->_post('city') : '';
			$country=$this->_post('country')? $this->_post('country') : '';
 			...
 			if ($cardid==0){
				...
				if($province){
					$userInfoExist=M('Userinfo')->where($infoWhere)->find();
					if ($userInfoExist){
						$userInfoexp=M('Userinfoexp')->where(array('uid' => $userInfoExist['id']))->find();
						if($userInfoexp){
							$userInfoexp=M('Userinfoexp')->where(array('uid' => $userInfoExist['id']))->save(array('province'=>$province,'city'=>$city,'country'=>$country));
						}
						else{
							$userInfoexp=M('Userinfoexp')->add(array('uid'=>$userInfoExist['id'],'province'=>$province,'city'=>$city,'country'=>$country));
						}
					}
				}
 				...
 				if($cardinfo){ //如果Member_card_create 不为空，说明领过卡，但是可以修改会员信息
 					...
 					if(M('Userinfo')->where($update)->save($data)){
						if($province){
							$userInfoExist=M('Userinfo')->where($update)->find();
							if ($userInfoExist){
								$userInfoexp=M('Userinfoexp')->where(array('uid' => $userInfoExist['id']))->find();
								if($userInfoexp){
									$userInfoexp=M('Userinfoexp')->where(array('uid' => $userInfoExist['id']))->save(array('province'=>$province,'city'=>$city,'country'=>$country));
