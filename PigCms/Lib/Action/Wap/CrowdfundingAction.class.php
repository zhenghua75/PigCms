...
class CrowdfundingAction extends WapAction{
	...
	public function home(){
		$where 		= array('token'=>$this->token,'status'=>array('eq',3));
	...
	public function pay(){
		...
		if(empty($orderid)){
			...
			if(M('Crowdfunding_order')->add($_POST)){
				$this->success('提交成功，正在跳转支付页面..',U('Alipay/pay',array('from'=>'Crowdfunding','orderName'=>$_POST['order_name'],'single_orderid'=>$_POST['orderid'],'token'=>$_POST['token'],'wecha_id'=>$_POST['wecha_id'],'price'=>$reward['price'])));
			}else{
			...
			if($order){
				$this->success('提交成功，正在跳转支付页面..',U('Alipay/pay',array('from'=>'Crowdfunding','orderName'=>$order['order_name'],'single_orderid'=>$order['orderid'],'token'=>$order['token'],'wecha_id'=>$order['wecha_id'],'price'=>$order['price'])));
			}else{
				$this->error('无效的订单');
			}
		}
...
/*		$act 		= $_GET['act'];
	...
	public function checkStatus($info){
		...
		if($end <= time() || $info['status']==4 || $is_max){
			M('Crowdfunding')->where(array('token'=>$this->token,'id'=>$info['id'],'status'=>array('lt',4)))->save(array('status'=>4));
			return true;
		}else{
			return false;
		}
	...
	public function getCounter($id,$name=''){
		...
		$arr = array(
			...
			'focusCounter' 	=> array(
				'praise'	=> M('Crowdfunding')->where($where)->getField('praise'),
				'focus'		=> M('Crowdfunding')->where($where)->getField('focus'),
			)
		);
	...
	public function getCrowdfunding($id){
		$where 	= array('token'=>$this->token,'id'=>$id,'status'=>array('gt',2));
