...
class ForumAction extends WapAction{
	...
	public function _initialize(){
		...
		$token = $this->token;
		$fid=$_GET['fid'];
		$count = M('Forum_config')->where(array('isopen'=>'1','token'=>$token))->count();
		$isopen = M('Forum_config')->field('isopen,bgurl')->where(array('fid'=>$fid,'token'=>$token))->find();
		...
		$userInfoex = M('User_extinfo')->field('truename')->where(array('uid' => $userInfo['id'],'token' => $this->token))->find();
		$this->truename=$userInfoex['truename'];
	...
	public function index(){
		...
		$token = $this->token;
		$forumhome = M('Forum_home')->where("token = '$token'")->find();
		$config = M('Forum_config')->where("token = '$token' and isopen = '1'")->select();
		
		$count[]=array();
		foreach ($config as $cf) {
			$where = array('isopen'=>'1','token'=>$token,'fid'=>$cf['id']);
			$count[$cf['id']] = M('Forum_topics')->where( $where )->count();
		}
		//提示
		if(!$this->isSubscribe()){
			$this->memberNotice();
	...
	public function board(){
		...
		$token = $this->token;
		$fid=$_GET['fid'];
		$where = array('status'=>'1','token'=>$token,'fid'=>$fid);
		...
		//增加浏览量
		if(!cookie('pv')){
			M('Forum_config')->where(array('token'=>$token,'fid'=>$fid))->setInc('pv');
			cookie('pv','1',24*60*60);
		}
		$config = M('Forum_config')->where(array('token'=>$token,'id'=>$fid))->find();
	...
	public function add(){
		...
		$fid=$_GET['fid'];
		if($uid == ''){
			$this->error("请先登录",U("Index/memberLogin", array("token" => $this->token)),false,1);
		}
	...
	public function checkAdd(){
		...
		if($userinfo['wechaname']){
			$data['uname']=$userinfo['wechaname'];
		}elseif($this->truename){
			$data['uname']=$this->truename;
		}else{
			$data['uname'] = '游客';
		}
		...
		$conf = M('Forum_config')->field('ischeck')->where("token ='$token' and id='$fid'")->find();
			...
			if(session('forumAdd')+60 < time()){
				...
				session('forumAdd',time());
					if($conf['ischeck'] == 1){
						...
						$this->redirect(U('Forum/board',array('wecha_id'=>$data['uid'],'token'=>$data['token'],'fid'=>$fid)));
					}
	...
	public function joinAjax(){
		...
		$info = M('Forum_topics')->field('joinid')->where("id = $id")->find();

		if($info['joinid'] == ''){
		
			$data['joinid'] = $this->_post('uid');
			$boo = M('Forum_topics')->where("id = $id")->setField($data);

		}else{
		
			$joinid = explode(',',$info['joinid']);
			if(in_array($this->_post('uid'),$joinid)){
			
				unset($joinid[array_search($this->_post('uid'),$joinid)]);
				$data['joinid'] = implode(',',$joinid);
				$boo = M('Forum_topics')->where("id = $id")->setField($data);

			}else{
			
				$data['joinid'] = $info['joinid'].','.$this->_post('uid');
				$boo = M('Forum_topics')->where("id = $id")->setField($data);
			}
		}
	...
	public function comment(){
		...
		foreach($comment as $k=>$v){
			...
			if($v['replyid'] != NULL){
				$reuid = $v['replyid'];
				$userinfo = M('Userinfo')->where("wecha_id = '$reuid' AND token = '$token'")->find();
				if($userinfo['wechaname']){
					$comment[$k]['reuname']=$userinfo['wechaname'];
				}elseif($userinfo['id']){
					$userInfoex = M('User_extinfo')->field('truename')->where(array('uid' => $userinfo['id'],'token' => $this->token))->find();
					$comment[$k]['reuname']=$userInfoex['truename'];
				}else{
					$comment[$k]['reuname'] = '游客';
				}	
			}
		}
		$config = M('Forum_config')->where(array('token'=>$token,'id'=>$topics['fid']))->find();

		$favourid=split(',', $topics['favourid']);
		$favourimg=array();
		foreach($favourid as $wid){
			$user1 = M('Userinfo')->where("(wecha_id = '$wid' or twid='$wid') AND token = '$token'")->find();
			$favourimg[$wid]=$user1['portrait'];
		}

		$likeid=split(',', $topics['likeid']);
		$likeimg=array();
		foreach($likeid as $wid){
			$user1 = M('Userinfo')->where("(wecha_id = '$wid' or twid='$wid') AND token = '$token'")->find();
			$likeimg[$wid]=$user1['portrait'];
		}

		$joinid=split(',', $topics['joinid']);
		$joinimg=array();
		foreach($joinid as $wid){
			$user1 = M('Userinfo')->where("(wecha_id = '$wid' or twid='$wid') AND token = '$token'")->find();
			$joinimg[$wid]=$user1['portrait'];
		}
		...
		$this->assign('config',$config);
		$this->assign('favourimg',$favourimg);
		$this->assign('likeimg',$likeimg);
		$this->assign('joinimg',$joinimg);
		$this->display();
	...
	public function checkCommentAdd(){
		...
		$comment = M('Forum_comment');
			...
			if($comment->add($data)){
				...
				if($userinfo['wechaname']){
					$message['touname']=$userinfo['wechaname'];
				}elseif($this->truename){
					$message['touname']=$this->truename;
				}else{
					$message['touname'] = '游客';
				}
