...
class ClassifyAction extends UserAction{
	...
	public function index(){
        ...
        session('token',$token);
        $grouptype=$this->_get('grouptype','trim');	
	...
	public function del(){
		...
		if(D(MODULE_NAME)->where($where)->delete()){
			...
			$this->success('操作成功',U(MODULE_NAME.'/index',array('fid'=>$_GET['fid'],'token'=>session('token'),'grouptype'=>session('grouptype'))));
		}else{
			$this->error('操作失败',U(MODULE_NAME.'/index',array('fid'=>$_GET['fid'],'token'=>session('token'),'grouptype'=>session('grouptype'))));
		}
	}
	//
	public function insert(){
		...
		if($db->create()===false){
			...
			$id=$db->add();
			if($id){
				$this->success('操作成功',U(MODULE_NAME.'/index',array('fid'=>$_POST['fid'],'token'=>session('token'),'grouptype'=>session('grouptype'))));
			}else{
				$this->error('操作失败',U(MODULE_NAME.'/index',array('fid'=>$_POST['fid'],'token'=>session('token'),'grouptype'=>session('grouptype'))));
			}
		}
	}
	public function upsave(){
		...
		if($fid == ''){
			$this->all_save('','/index?token='.session('token').'&grouptype='.session('grouptype'));
		}else{
			$this->all_save('','/index?fid='.$fid.'&token='.session('token').'&grouptype='.session('grouptype'));
		}
	}
	...
	public function chooseTpl(){
		...
		include('./PigCms/Lib/ORG/index.Tpl.php');
		include('./PigCms/Lib/ORG/cont.Tpl.php');
		include('./PigCms/Lib/ORG/custom.Tpl.php');
		...
		$customTpl = array_reverse($customTpl);
		foreach($customTpl as $key=>$val){
				if($val['tpltypeid'] == $tpid){
					$info['tplview3'] = $val['tplview'];
					$info['custom_user'] = $val['user'];
				}
			}

		$logintpl = array_reverse($logintpl);
		$logintpid = $this->_get('tpid','intval');
		foreach($logintpl as $key=>$val){
			if($val['tpltypeid'] == $logintpid){
				$info['tplview4'] = $val['tplview'];
			}
		}
            
		$this->assign('contTpl',$contTpl);
		$this->assign('tpl',$tpl);
		$this->assign('customTpl',$customTpl);
		$this->assign('logintpl',$logintpl);
		$this->assign('info',$info);
		$this->display();
	}
