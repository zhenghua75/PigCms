...
class Member_cardAction extends UserAction{
	...
	*/
	...
	public function coupons_set(){
		...
		if(true || $this->wxuser['winxintype'] == 3){
...
//删除记录
	...
	public function center(){
		...
		//$show       = $Page->show();
		$userwhere['token']=$this->token;
		$userwhere['username']=array('neq','');
		$alluser 	= M('Userinfo')->where($userwhere)->select();
		$wechalist=array();
		foreach($alluser as $key=>$value){
			if($value['wecha_id'] && $value['twid']){
				$where['wecha_id']	= array('in',$value['wecha_id'].','.$value['twid']);
			}elseif($value['wecha_id'] && !$value['twid']){
				$where['wecha_id']	= $value['wecha_id'];
			}elseif(!$value['wecha_id'] && $value['twid']){
				$where['wecha_id']	= $value['twid'];
			}
			$cardinfo = M('Member_card_create')->where($where)->find();
			if(empty($value['wecha_id'])&&empty($value['twid'])){
				continue;
			}
			if(!in_array($value['wecha_id'].$value['twid'], $wechalist)){
				array_push($wechalist, $value['wecha_id'].$value['twid']);
				$allcard[$value['id']]['score'] 	= $value['total_score'];
				$allcard[$value['id']]['expense'] 	= $value['expensetotal'];
				$allcard[$value['id']]['balance'] 	= $value['balance'];
				$allcard[$value['id']]['createtime']= $value['getcardtime'];
				$allcard[$value['id']]['truename']	= $value['truename'];
				$allcard[$value['id']]['wechaname']	= $value['wechaname'];
				$allcard[$value['id']]['username']	= $value['username'];
				$allcard[$value['id']]['tel']		= $value['tel'];
				if($cardinfo){
					$allcard[$value['id']]['number']	= $cardinfo['number'];
					$allcard[$value['id']]['card_name'] = M('Member_card_set')->where(array('token'=>$this->token,'id'=>$cardinfo['cardid']))->getField('cardname');
				}else{
					$allcard[$value['id']]['number']='';
					$allcard[$value['id']]['card_name']='';
				}
				
				if($province){
					$userinfoexp = M('Userinfoexp')->where(array('uid'=>$value['id'],'province'=>$province,'city'=>$city,'country'=>$country))->find();
					if(!$userinfoexp){
						unset($allcard[$value['id']]);
					}else{
						$provincename=M('China_address')->field('region_name')->where(array('region_type'=>1,'region_id'=>$userinfoexp['province']))->find();
						$cityname=M('China_address')->field('region_name')->where(array('region_type'=>2,'region_id'=>$userinfoexp['city']))->find();
						$countryname=M('China_address')->field('region_name')->where(array('region_type'=>3,'region_id'=>$userinfoexp['country']))->find();
						$allcard[$value['id']]['area'] = $provincename['region_name'].$cityname['region_name'].$countryname['region_name'];
					}
				}else{
					$userinfoexp = M('Userinfoexp')->where(array('uid'=>$value['id']))->find();
					if($userinfoexp){
						$provincename=M('China_address')->field('region_name')->where(array('region_type'=>1,'region_id'=>$userinfoexp['province']))->find();
						$cityname=M('China_address')->field('region_name')->where(array('region_type'=>2,'region_id'=>$userinfoexp['city']))->find();
						$countryname=M('China_address')->field('region_name')->where(array('region_type'=>3,'region_id'=>$userinfoexp['country']))->find();
						...
						$allcard[$value['id']]['area'] = '';
					}
				}
			}
		}
		$Page = new Page(count($allcard),15);
		...
		$this->assign('cards',$cards);
		$this->assign('allCard',array_slice($allcard,$Page->firstRow,$Page->listRows));
		$this->assign('page',$Page->show());
