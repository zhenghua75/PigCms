...
function musicUpload(domid,width,height,token){
	art.dialog.data('width', width);
	art.dialog.data('height', height);
	art.dialog.data('domid', domid);
	art.dialog.data('lastpic', $('#'+domid).val());
	art.dialog.open('?g=User&m=Upyun&a=upload&token='+token+'&width='+width,{lock:true,title:'上传音乐',width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.45});
}
...
function memberCardRecharge(uid,cardid){
	art.dialog.data('uid', uid);
	art.dialog.data('cardid', cardid);
	art.dialog.open('?g=User&m=Member_card&a=recharge&uid='+uid+'&cardid='+cardid,{lock:true,title:'充值会员卡',width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.45});
}
